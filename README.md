...
5. ...
   ...
   def find_most_affected_firm_during_period(start_year, end_year):
    ...
    most_affected_conm = filter_by_col_value(df_end, 'gvkey', most_affexted_gvkey)['conm'].values[0]
    max_drop = abs(percentage_drop.min())

    print(f"The firm most affected between {start_year} and {end_year} is {most_affected_conm} (gvkey: {most_affexted_gvkey}) with a drop of {max_drop}%.")
   ```
6. ...
